 ...
 *
 * Unlike [TransformedFxProjection] (one entity per bucket), this map places each entity under
 ...
 * The [close] method cancels all entity subscriptions and clears entries-changed listeners.
 *
...
class TransformedFxMultiKeyProjection<K : Comparable<K>, PK : Comparable<PK>, E, V : Any>(
    ...
    /**
     * Cancels all per-entity mutation subscriptions, clears entries-changed listeners, and releases
     * resources. Safe to call before the first access (no-op when not yet initialized). After
     * closing, in-place entity mutations no longer trigger re-bucketing and no further
    ...
    companion object {
        private const val READ_ONLY_MESSAGE = "TransformedFxMultiKeyProjection is read-only"
    }
     ...
     * `val byGenre: ObservableMap<String, GenreStats> by fxMultiKeyProjection(src, keys, transform)`.
     */
    operator fun getValue(thisRef: Any?, property: KProperty<*>): TransformedFxMultiKeyProjection<K, PK, E, V> {
        initialize()
        return this
    }
}