...
/**
 * A read-only value-transformed view of a [MultiKeyProjectionMap] that derives a `Map<PK, V>` by
 ...
 *
 * This decorator wraps a [MultiKeyProjectionMap] and registers on its `onBucketsChanged` signal to
 ...
 *
 * **Single-subscriber:** The `onBucketsChanged` slot on the backing [MultiKeyProjectionMap] is
 * owned by this decorator. Registering another `onBucketsChanged` on the same backing map after
 * constructing a [TransformedMultiKeyProjectionMap] will overwrite this decorator's registration,
 ...
 * @param backing the underlying [MultiKeyProjectionMap] whose buckets are transformed
 ...
 */
internal class TransformedMultiKeyProjectionMap<K : Comparable<K>, PK : Comparable<PK>, E : IdentifiableEntity<K>, V>(
    private val backing: MultiKeyProjectionMap<K, PK, E>,
