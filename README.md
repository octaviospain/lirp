...
- **JSON persistence** — debounced file writes via `JsonFileRepository`. `LirpEntitySerializer` round-trips reactive entities with zero reflection when lirp-ksp is applied, and falls back to a reflection-based accessor (no `--add-opens` required) when an entity's module deliberately omits lirp-ksp — so JSON persistence works with or without code generation
